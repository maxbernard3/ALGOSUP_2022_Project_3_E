...
module Filter =
    ...
    let addTwinWaves (x:List<float>) (y:List<float>) ratio = 
    ...
    let createEcho (startIndex:int) (endIndex:int) (delay:float) (nbEcho:int) (x:List<float>) = //All of the sound echoed
    ...
    let createFlanging (data:List<float>) (start:float) (ending:float) (delay:float) (rate:float) repNumber sampleRate = 
        let mutable delarate = delay
        let mutable rep = repNumber
        let mutable actualData = data
        while rep > 0 do
            actualData <- createDelay data start ending dela sampleRate
            delarate <- delarate + dela/rate
            rep <- rep - 1
        actualData